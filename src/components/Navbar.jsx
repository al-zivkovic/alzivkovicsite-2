...
const Navbar = () => {
    ...
    return (
        <>
            ...
            <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-20 bg-secondary border-t border-gray-600">
                <div className="flex items-center justify-around py-3 px-4">
                    <Link to="#home" onClick={(e) => { e.preventDefault(); scrollToId('home'); }} className="flex flex-col items-center gap-1 text-white" aria-label="Home" title="Home">
                        <HomeIcon className="text-2xl"/>
                        <span className="text-xs">Home</span>
                    </Link>
                    <Link to="#experience" onClick={(e) => { e.preventDefault(); scrollToId('experience'); }} className="flex flex-col items-center gap-1 text-white" aria-label="Experience" title="Experience">
                        <WorkHistoryIcon className="text-2xl"/>
                        <span className="text-xs">Experience</span>
                    </Link>
                    <Link to="#projects" onClick={(e) => { e.preventDefault(); scrollToId('projects'); }} className="flex flex-col items-center gap-1 text-white" aria-label="Projects" title="Projects">
                        <CodeIcon className="text-2xl"/>
                        <span className="text-xs">Projects</span>
                    </Link>
                    <Link to="https://github.com/al-zivkovic" className="flex flex-col items-center gap-1 text-white" aria-label="GitHub" title="GitHub">
                        <GitHubIcon className="text-2xl"/>
                        <span className="text-xs">GitHub</span>
                    </Link>
                    <Link to="https://www.linkedin.com/in/aleksandar-zivkovic-04bb2620b/" className="flex flex-col items-center gap-1 text-white" aria-label="LinkedIn" title="LinkedIn">
            ...
            <nav className="hidden sm:block group w-fit transition-all duration-300 ease-out flex items-center p-5 fixed top-5 z-20 bg-secondary rounded-[50px] text-white shadow-lg shadow-black/50">
                <div className="flex items-center gap-5">
                    ...
                    <Link to="https://www.linkedin.com/in/aleksandar-zivkovic-04bb2620b/" className="group flex items-center gap-2" aria-label="LinkedIn" title="LinkedIn">
